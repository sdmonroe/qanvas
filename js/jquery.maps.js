...
jQuery.googleMap = {
	...
	readFromGeo: function(elem) {
		var latElem = jQuery(".latitude", elem)[0];
		var lngElem = jQuery(".longitude", elem)[0];
		if (latElem && lngElem) {
			return { lat:parseFloat(jQuery(latElem).attr("title")), lng:parseFloat(jQuery(lngElem).attr("title")), txt:jQuery(elem).attr("title") }
		} else {
			return null;
		}
	},
